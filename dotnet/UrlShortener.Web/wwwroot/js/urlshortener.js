// URL Shortener Frontend JavaScript for ASP.NET Core

class URLShortener {
    constructor() {
        this.baseURL = window.location.origin;
        this.apiURL = window.API_BASE_URL || 'http://localhost:5080';
    ...
    async handleShortenUrl(event) {
        ...
        try {
            ...
            const result = await this.makeRequest(`${this.apiURL}/api/shorten`, {
                method: 'POST',
    ...
    async handleExpandUrl(event) {
        ...
        try {
            ...
            const result = await this.makeRequest(`${this.apiURL}/api/expand/${shortCode}`);
    ...
    displayShortenResult(result) {
        if (!this.shortUrlDisplay || !this.originalUrlDisplay || !this.shortenResult) return;
    ...
    displayExpandResult(result) {
        if (!this.originalUrlLink || !this.shortCodeDisplay || !this.createdAtDisplay || !this.expandResult) return;
    ...
    async copyToClipboard() {
        if (!this.shortUrlDisplay || !this.copyBtn) return;
    ...
    // Load all URLs
    async loadAllUrls() {
        if (!this.urlsList) return;
        
        try {
            const result = await this.makeRequest(`${this.apiURL}/api/urls`);
