...
public class SqlServerDatabase : IUrlDatabase
{
    ...
    public async Task<UrlMapping> CreateUrlAsync(string shortCode, string originalUrl)
    {
        ...
        var command = new SqlCommand(@"
            ...
            OUTPUT INSERTED.id
            VALUES (@shortCode, @originalUrl)", connection);
    ...
    public async Task<UrlMapping?> GetUrlAsync(string shortCode)
    {
        ...
        var command = new SqlCommand("SELECT * FROM urls WHERE short_code = @shortCode", connection);
        command.Parameters.AddWithValue("@shortCode", shortCode);
    ...
    public async Task<List<UrlMapping>> GetAllUrlsAsync()
    {
        ...
        var command = new SqlCommand("SELECT * FROM urls ORDER BY created_at DESC", connection);
    ...
    {
        // SqlConnection implements IDisposable, connections are managed per request
